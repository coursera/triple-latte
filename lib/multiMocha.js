...
function outputResults(totals) {
  ...
  console.log(
    ...
    [
      ...
      (color.red(util.format('%s failing', (totals.errored.length + totals.failed.length).toString()) + (totals.errored.length + totals.failed.length > 0 ? ' - details in triple-latte.fail.log' : ''))),
