...
exports.runner = function(config, configFile) {
  ...
  async.mapLimit(tests, workers, function(_tests, done) {
    ...
    oneTestSpawn = fork(path.join(__dirname, 'cli.js'), args.concat(split > 0 ? _tests : [_tests]), {silent:false});
    ...
    if (/xunit.js$/.test(reporter)) {
      ...
      if (totals.failed.length == 0) {
        ...
        console.log(color.red('  1 test failed') + ' - details in triple-latte.log\n');
      } else {
        console.log(color.red(util.format('  %s tests failed', totals.failed.length)) + ' - details in triple-latte.log\n');
      }
