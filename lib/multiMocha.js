...
exports.runner = function(config, configFile) {
  ...
  var totals = {passed:[], failed:[], std:{out:'', err:''}};
  ...
  async.mapLimit(tests, workers, function(_tests, done) {
    ...
    oneTestSpawn = fork(path.join(__dirname, 'cli.js'), args.concat(split > 0 ? _tests : [_tests]), {silent:true});
    ...
    oneTestSpawn.on('exit', function(code) {
      if (totals.std.err.length) {
        totals.failed.push({title:'node error', file:_tests});
        process.stderr.write(totals.std.err);
      }

      if (totals.std.out.length) {
        process.stdout.write(totals.std.out);
      }
    ...
    if (/xunit.js$/.test(reporter)) {
      if (totals.failed.length && totals.std.err.length) {
        console.log('<testsuite name="' + totals.failed[0].title + '" tests="1" failures="0" errors="1" />');
      }
      ...
      } else if (totals.failed.length == 1) {
        console.log(color.red('  1 test failed') + ' - details in triple-latte.fail.log\n');
      } else {
        console.log(color.red(
          util.format('  %s tests failed', totals.failed.length)) + ' - details in triple-latte.fail.log\n');
      }
